...
void ArraySymmetry(vector<vector<int>> a, int k, int n) //bool ArraySymmetry(vector<vector<int>> a, int k, int n, int i, int j, bool symm)
{
	/*if (i < k - 1) 
	{
		if (j < n - 1)
		{
			...
			else { return ArraySymmetry(a, k, n, i, j + 1, symm); }
		}
		else
		{
			if (a[i][j] != a[k - i - 1][n - j - 1])
			...
			else { return ArraySymmetry(a, k, n, i + 1, 0, symm); }
		}
	}*/
	bool symm = true;
	if (k != n)
